				...
				if [[ -z "$_is_update_required" ]]; then
					...
					if [[ -z "$_is_update_required" ]]; then
						...
						if [[ -n "$_is_frash_install" ]]; then
							...
							echo -e "Follow me!"
							echo
							echo -e "\t+ Check out my website at $(tblue)https://www.KhaiVpn767.com$(treset)"
							echo -e "\t+ Follow me on Telegram: $(tblue)https://t.me/KhaiVpn767$(treset)"
